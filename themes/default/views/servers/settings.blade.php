        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-xl-6">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="float-right card-title"><i title="Created at"
                                    class="mr-2 fas fa-calendar-alt"></i><span>{{ $server->created_at->isoFormat('LL') }}</span>
                            </h5>
                                        ...
                                        <div class="modal-dialog">
                                            <div x-data class="modal-content">
                                                ...
                                                <div class="modal-body card-body">
                                                    <strong>{{ __('Current Product') }}: </strong>
                                                    {{ $server->product->name }}
                                                    <br>
                                                    <br>

                                                    <form
                                                        action="{{ route('servers.upgrade', ['server' => $server->id]) }}"
                                                ...
                                                <div class="modal-footer card-body">
                                                    <button x-ref="upgradeSubmit" type="submit"
                                                        class="btn btn-primary upgrade-once" style="width: 100%"
                                                        disabled><strong>{{ __('Change Product') }}</strong></button>
                                                </div>
                                                </form>
                                            </div>
                                        </div>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="DeleteModalLabel">{{ __('Delete Server') }}
                                                </h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                    aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form class="d-inline" method="post"
                                                    ...
                                                    <button data-toggle="popover" data-trigger="hover"
                                                        data-placement="top"
                                                        class="mr-1 btn btn-danger">{{ __('Delete') }}</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="col-xl-6">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title"><i class="mr-2 fas fa-flag"></i>{{ __('Server Billing Priority') }}
                            </h5>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-lg-8">
                                    <span style="" class="d-inline-block">
                                        {{ $server->effective_billing_priority->label() }} -
                                        ({{ $server->effective_billing_priority->description() }})
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="card-footer">
                            <div class="text-center col-md-12">
                                ...
                                <button type="button" data-toggle="modal" data-target="#BillingPriorityModal"
                                    class="btn btn-info btn-md">
                                    <i class="mr-2 fas fa-flag"></i>
                                    <span>{{ __('Change Billing Priority') }}</span>
                                </button>
                                <!-- Billing Priority Modal -->
                                <div class="modal fade" id="BillingPriorityModal" tabindex="-1" role="dialog"
                                    aria-labelledby="BillingPriorityModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div x-data class="modal-content">
                                            <div class="modal-header card-header">
                                            ...
                                            <div class="modal-body card-body">
                                                <form
                                                    action="{{ route('servers.updateBillingPriority', ['server' => $server->id]) }}"
                                                    method="POST" id="billing_priority_form">
                                                    @csrf
                                                    @method('PATCH')
                                                    <select
                                                        ...
                                                        class="form-input2 form-control">
                                                        <option value="">{{ __('Select the billing priority') }}
                                                        </option>
                                                        @foreach (App\Enums\BillingPriority::cases() as $priority)
                                                            <option value="{{ $priority->value }}"
                                                                @selected($server->effective_billing_priority == $priority)>
                                                                {{ $priority->label() }} -
                                            ...
                                            <div class="modal-footer">
                                                <button x-ref="prioritySubmit" type="submit"
                                                    form="billing_priority_form"
                                                    class="btn btn-primary billing_priority_submit w-100" disabled>
                                                    {{ __('Update') }}
                                                </button>
                                            </div>
                                        </div>

    ...
    <section class="content-header">
        ...
        @if (!empty($headFileMissing))
            <div class="alert alert-warning" role="alert">
                <b><i class="fas fa-exclamation-triangle"></i> {{ __('Version metadata unavailable:') }}</b><br>
                {{ __('The .git/HEAD file could not be found, so branch information may display as unknown.') }}
            </div>
        @endif
    </section>
    <!-- END CONTENT HEADER -->
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                    ...
                    <div class="info-box">
                        ...
                        <div class="info-box-content">
                            <span class="info-box-text">{{ __('Servers') }}
                                <i class="mr-4 fas fa-info-circle" data-toggle="popover" data-trigger="hover"
                            ...
                            <span
                                class="info-box-number">{{ $counters['servers']->active }}/{{ $counters['servers']->total }}</span>
                        </div>
                        <!-- /.info-box-content -->
                    </div>
                    <!-- /.info-box -->
                ...
                <div class="col-12 col-sm-6 col-md-3">
                    <div class="info-box">
                        ...
                        <div class="info-box-content">
                            ...
                            <span
                                class="info-box-number">{{ $counters['users']->active }}/{{ $counters['users']->total }}</span>
                        </div>
                        <!-- /.info-box-content -->
                    </div>
                    <!-- /.info-box -->
                ...
                <div class="col-12 col-sm-6 col-md-3">
                    <div class="info-box">
                        <span class="info-box-icon bg-warning elevation-1"><i class="text-white fas fa-coins"></i></span>
                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-footer">
                            <span><i
                                    class="mr-2 fas fa-sync"></i>{{ __('Last updated :date', ['date' => $syncLastUpdate]) }}</span>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header">
                                            ...
                                            @foreach ($tickets as $ticket_id => $ticket)
                                                <tr class="text-nowrap">
                                                    <td><a class="text-info"
                                                            href="{{ route('admin.ticket.show', ['ticket_id' => $ticket_id]) }}">#{{ $ticket_id }}
                                                            - {{ $ticket->title }}</td>
                                                    <td><a
                                                            href="{{ route('admin.users.show', $ticket->user_id) }}">{{ $ticket->user }}</a>
                                                    </td>
                                                    <td><span
                                                            class="badge {{ $ticket->statusBadgeColor }}">{{ $ticket->status }}</span>
                                                    </td>
                        ...
                        <div class="card-body p-4">
                            <div class="row text-center">
                                <div class="col-4">
                                    ...
                                    <div class="h5 font-weight-bold mb-0">v{{ config('app.version') }}</div>
                                </div>
                                <div class="col-4 border-left border-right"
                                    style="border-color: rgba(255,255,255,0.1) !important;">
                                    <div class="text-uppercase text-muted small font-weight-bold mb-2">{{ __('Branch') }}
                                    </div>
                                    <div class="h5 font-weight-bold mb-0">{{ $branch }}</div>
                                </div>
                                <div class="col-4">
                                    <div class="text-uppercase text-muted small font-weight-bold mb-2">{{ __('Commit') }}
                                    </div>
                                    <div class="h5 font-weight-bold mb-0" style="font-family: monospace;">
                                        {{ $commit }}</div>
                                </div>
                            </div>
                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="py-1 card-body">
                                ...
                                <table class="table">
                                    <thead>
                                        <tr class="text-nowrap">
                                            ...
                                            <th>{{ $credits_display_name . ' ' . __('Usage') . ' (' . __('per month') . ')' }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($nodes as $nodeID => $node)
                                            <tr>
                                                ...
                                                <td>{{ Currency::formatForDisplay($node->activeEarnings) }}/{{ Currency::formatForDisplay($node->totalEarnings) }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            <td class="text-nowrap" colspan="2"><span
                                                    style="float: right; font-weight: 700">{{ __('Total') }}
                                                    ({{ __('active') }}/{{ __('total') }}):</span></td>
                                            <td>{{ $counters['servers']->active }}/{{ $counters['servers']->total }}</td>
                                            <td>{{ $counters['totalUsagePercent'] }}%</td>
                                            <td>{{ Currency::formatForDisplay($counters['earnings']->active) }}/{{ Currency::formatForDisplay($counters['earnings']->total) }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                            ...
                            <hr
                                style="width: 100%; height:2px; border-width:0; background-color:#6c757d; margin-top: 0px;">
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="card-title ">
                                    <span><i
                                            class="mr-2 fas fa-file-invoice-dollar"></i>{{ __('Latest payments') }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="py-1 card-body">
                            <div class="row">
                                @if ($counters['payments']['lastMonth']->count())
                                    <div class="col-md-6" style="border-right:1px solid #6c757d">
                                        <span
                                            style="margin:auto; display:table; font-size: 18px; font-weight:700">{{ __('Last month') }}:
                                ...
                                @endif
                                <span
                                    style="margin:auto; display:table; font-size: 18px; font-weight:700">{{ __('This month') }}:
                                    <i data-toggle="popover" data-trigger="hover" data-html="true"
                ...
                <div class="card">
                    ...
                    <div class="py-1 card-body">
                        @if ($counters['taxPayments']['lastYear']->count())
                            <span
                                style="margin:auto; display:table; font-size: 18px; font-weight:700">{{ __('Last year') }}:
                                <i data-toggle="popover" data-trigger="hover" data-html="true"
                            ...
                            <hr
                                style="width: 100%; height:2px; border-width:0; background-color:#6c757d; margin-top: 0px; margin-bottom: 8px">
                        @endif
